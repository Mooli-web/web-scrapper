...
import { Play, Square, Radio, Terminal, Sparkles, CheckCircle2, AlertTriangle, Layers, Clock, Copy, Trash2, ArrowDown, Smartphone } from 'lucide-react';
import { startCrawler, stopCrawler } from '../api';

export default function LiveMissionControl({ onStatusChange }) {
  const [telemetry, setTelemetry] = useState({
    status: 'idle',
    active_store: 'دیوار (divar.ir)',
    ...
    errors_count: 0,
    latest_log: 'موتور خزش دیوار آماده است.'
  });
  ...
  useEffect(() => {
    ...
    const streamInterval = setInterval(async () => {
      if (!active) return;
      try {
        const token = localStorage.getItem('divar_auth_token');
    ...
    <div className="glass-card rounded-3xl p-6 border border-slate-800 space-y-5 bg-gradient-to-b from-slate-900/90 to-slate-950/90">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-base font-black text-white">مرکز فرماندهی خزش پیوسته دیوار (Divar Mission Control)</h3>
              <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-black border ${
                isRunning 
                  ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30'
                  : 'bg-slate-800 text-slate-400 border-slate-700'
              }`}>
                {isRunning ? '🟢 در حال خزش پیوسته (۲۴/۷)' : '⚪ متوقف (Idle)'}
              </span>
            </div>
        ...
        <div className="flex items-center gap-2">
          {!isRunning ? (
            <button
              ...
              className="flex items-center gap-2 px-5 py-2.5 rounded-2xl bg-gradient-to-r from-rose-600 to-rose-500 hover:from-rose-500 hover:to-rose-400 text-white font-black text-xs shadow-lg shadow-rose-600/30 transition cursor-pointer disabled:opacity-50"
            >
              <Play className="w-4 h-4 fill-white" />
              <span>شروع پایش ۲۴ ساعته دیوار</span>
            </button>
          ) : (
            <button
              onClick={handleStop}
              disabled={loadingAction}
              className="flex items-center gap-2 px-5 py-2.5 rounded-2xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-black text-xs border border-slate-700 transition cursor-pointer disabled:opacity-50"
            >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
        <div className="p-3.5 rounded-2xl bg-slate-950/90 border border-slate-800">
          <span className="text-[10px] text-slate-400 block">آگهی‌های اسکن‌شده</span>
          <strong className="text-base font-black text-white font-mono">{Number(telemetry.posts_scanned).toLocaleString('fa-IR')}</strong>
        </div>

        <div className="p-3.5 rounded-2xl bg-slate-950/90 border border-slate-800">
          <span className="text-[10px] text-slate-400 block">آگهی‌های جدید</span>
          <strong className="text-base font-black text-emerald-400 font-mono">{Number(telemetry.new_posts_found).toLocaleString('fa-IR')}</strong>
        </div>

        <div className="p-3.5 rounded-2xl bg-slate-950/90 border border-slate-800">
          <span className="text-[10px] text-slate-400 block">تغییرات قیمت</span>
          <strong className="text-base font-black text-amber-400 font-mono">{Number(telemetry.price_changes_detected).toLocaleString('fa-IR')}</strong>
        </div>

        <div className="p-3.5 rounded-2xl bg-slate-950/90 border border-slate-800">
          <span className="text-[10px] text-slate-400 block">خطاهای ارتباطی</span>
          <strong className="text-sm font-black text-rose-400 font-mono">{telemetry.errors_count}</strong>
        </div>
      </div>
      ...
      <div className="rounded-2xl bg-slate-950 border border-slate-800 shadow-xl overflow-hidden">
        <div className="flex items-center justify-between px-4 py-2.5 bg-slate-900/90 border-b border-slate-800 text-[11px] text-slate-400">
          <div className="flex items-center gap-2 font-mono">
            ...
            <span className="font-bold text-white">کنسول زنده فعالیت موتور دیوار (Live Stream):</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`px-2.5 py-1 rounded-lg border text-[10px] font-semibold transition cursor-pointer flex items-center gap-1 ${
                autoScroll ? 'bg-rose-500/20 text-rose-400 border-rose-500/30' : 'bg-slate-800 text-slate-400 border-slate-700'
              }`}
            >
        ...
        <div ref={logContainerRef} className="h-64 overflow-y-auto p-4 font-mono text-[11px] space-y-1.5 text-right select-text" dir="ltr">
          {logs.length > 0 ? (
            ...
            <div className="text-slate-500 text-center py-20 font-sans text-xs">در انتظار شروع خزش دیوار...</div>
          )}
        </div>
      </div>
    </div>
