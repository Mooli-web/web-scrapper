...
STANDARD_CATEGORIES = {
    ...
    # accessories = لوازم جانبی: کالایی که «برای» یک دستگاه دیگر فروخته می‌شود
    #               (کیف و کاور تبلت/گوشی، محافظ صفحه، قلم لمسی، شارژر، کابل،
    #               بند ساعت، پایه). این‌ها محصول واقعی‌اند ولی هیچ‌کدام خودِ
    #               دستگاه نیستند؛ اگر به tablet/mobile بخورند، مدل قیمتِ کاورِ
    #               ۵ میلیونی را با تبلت ۳۰ میلیونی قاطی می‌کند.
    "accessories":  "لوازم جانبی",
    # other = متفرقه: روتر و تجهیزات شبکه، قاب/داک هارد و هر آنچه در دسته‌های
    #         بالا جا نمی‌شود.
    "other":        "متفرقه",
}
...
_LEADING_TITLE_RULES = [
    # لوازم جانبی — باید «پیش از» قواعد دستگاه‌ها بررسی شود: عنوانِ «کاور سامسونگ
    # مدل S Protective مناسب برای تبلت Galaxy Tab S8» کلمه‌ی تبلت را دارد، ولی
    # کالای فروخته‌شده کاور است نه تبلت.
    # «کیف پول سخت‌افزاری» (کیف پول رمز ارز) استثناست: خودش یک دستگاه است.
    (re.compile(r'^کیف(?! پول)|^کاور|^قاب(?!\s*(?:هارد|اس\s*اس\s*دی|ssd|باکس))|^محافظ\s*صفحه|^گلس\b|'
                r'^قلم\s*لمسی|^بند\s*ساعت|^پایه\s*نگهدارنده|^هولدر|^استند|'
                r'^شارژر|^کابل|^سیم(?!\s*کارت)|^آداپتور|^مبدل', re.I), "accessories"),
    # قاب/باکس هارد و داک = واسطِ ذخیره‌سازی، نه لوازم جانبیِ دستگاه → متفرقه
    (re.compile(r'^قاب\s*(?:هارد|اس\s*اس\s*دی|ssd)', re.I), "other"),
    ...
    (re.compile(r'^سیستم(?!\s*خنک)|^ریگ|^ماینر', re.I), "desktop-pc"),
    ...
    (re.compile(r'^خنک\s*کننده|^کولر|^واتر\s*کولر|^هیت\s*سینک|^هیتسینک|'
                r'^فن\s*(?:کیس|پردازنده|cpu)|^خمیر\s*سیلیکون|'
                r'^سیستم\s*خنک\s*کننده', re.I), "pc-parts"),
