...
logger = logging.getLogger("hub.sync.divar")
...
def ingest_divar_items(items: List[Dict[str, Any]]) -> int:
    """
    Ingests Divar ads with full descriptions, location districts, and condition specs.
    """
    if not items:
        return 0
    ...
    for it in items:
        try:
            item_id = str(it.get('token') or it.get('item_id') or it.get('id') or '')
            ...
            url = it.get('post_url') or it.get('url') or f"https://divar.ir/v/{item_id}"
            ...
            specs = extract_storage_and_specs(f"{clean_title} {desc}")
            ...
            listings.append((
                c_key, 'divar', item_id, clean_title, price,
                condition, 0, 0, 'آگهی‌دهنده دیوار', 'کاربر تاییدشده', 'تضمین سلامت توافقی',
                district, desc, specs_str, 0.0, 0, url, img, 0
            ))

            history.append((c_key, 'divar', price, condition))
            processed_ids.add(item_id)
        except Exception as e:
            logger.debug(f"Divar item parse note: {e}")
    ...
    logger.info(f"✅ [دیوار] ثبت دسته‌ای {len(listings)} آگهی با اطلاعات تکمیلی انجام شد.")
    return len(listings)
